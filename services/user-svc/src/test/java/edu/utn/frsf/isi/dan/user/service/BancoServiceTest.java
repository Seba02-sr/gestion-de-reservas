...
class BancoServiceTest {
  ...
  @Mock private BancoMapper bancoMapper;

  @InjectMocks private BancoService bancoService;
  ...
  void setUp() {
    ...
    bancoResponse = BancoResponse.builder().id(1).nombre("Banco Test").activo(true).build();
    bancoRequest = BancoRequest.builder().nombre("Banco Test").build();
  }
  ...
  void updateBanco_DeberiaRetornar400_CuandoIdEsNull() {
    assertThrows(
        IllegalArgumentException.class, () -> bancoService.updateBanco(null, bancoRequest));
  }
