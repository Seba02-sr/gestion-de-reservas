 ...
 *
 * <p>Estos tests verifican que: 1- Los endpoints respondan correctamente a las peticiones HTTP. 2-
 * Se validen correctamente los parametros de entrada. 3- Se manejen adecuadamente los errores y
 * excepciones. 4- Se devuelvan las respuestas esperadas.
 */
...
public class BancoControllerTest {
  ...
  @MockitoBean private BancoService bancoService;
  ...
  void setUp() {
    ...
    Mockito.when(bancoService.createBanco(Mockito.any(BancoRequest.class)))
        .thenReturn(bancoResponse);
  ...
  @Test
  @DisplayName(
      "Obtener todos los bancos cuando no hay bancos - debe retornar 200 y una lista vacía")
  ...
  void getBancoById_DebeRetornar400_CuandoIdEsNegativo() throws Exception {
    ...
    given(bancoService.getBancoById(-5))
        .willThrow(new EntityNotFoundException("Banco no encontrado"));

    mockMvc.perform(get("/bancos/-5")).andExpect(status().isBadRequest());
  }
  ...
  void getBancoById_DebeRetornar404_CuandoIdNoExiste() throws Exception {
    when(bancoService.getBancoById(999))
        .thenThrow(new EntityNotFoundException("Banco no encontrado"));
    mockMvc.perform(get("/bancos/999")).andExpect(status().isNotFound());
  ...
  void deleteBanco_DebeRetornar400_CuandoIdEsNegativo() throws Exception {
    mockMvc.perform(delete("/bancos/-3")).andExpect(status().isBadRequest());
  }
  ...
  void deleteBanco_DebeRetornar404_CuandoIdNoExiste() throws Exception {
    when(bancoService.deleteBanco(999))
        .thenThrow(new EntityNotFoundException("Banco no encontrado"));

    mockMvc.perform(delete("/bancos/999")).andExpect(status().isNotFound());
  ...
  void getBancosByNombre_DebeRetornar200() throws Exception {
    ...
    mockMvc
        .perform(get("/bancos/buscar").param("nombre", "Prueba"))
        .andExpect(status().isOk())
  ...
  void getBancosByNombre_DebeRetornar200_CuandoNoHayResultados() throws Exception {
    ...
    mockMvc
        .perform(get("/bancos/buscar").param("nombre", "Inexistente"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(0));
  ...
  void getBancosByNombre_DebeRetornar400_CuandoNombreEsVacio() throws Exception {
    mockMvc.perform(get("/bancos/buscar").param("nombre", "")).andExpect(status().isBadRequest());
  }
}
